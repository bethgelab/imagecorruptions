...
def snow(x, severity=1):
    ...
    snow_layer.motion_blur(radius=c[4], sigma=c[5],
    ...
    # The snow layer is cropped to the img dims
...
def pixelate(x, severity=1):
    ...
    x = x.resize((int(x_shape[1] * c), int(x_shape[0] * c)), PILImage.BOX)

    x = x.resize((x_shape[1], x_shape[0]), PILImage.NEAREST)
